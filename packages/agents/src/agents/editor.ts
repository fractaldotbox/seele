...
import {
  getTransaction,
  getTransactionsWithAddress,
} from "@seele/data-fetch/blockscout/index";
...
import { addressEditor } from "./address-book.js";
...
export const checkSubmissions = async () => {
  const results = await getTransactionsWithAddress(
    addressEditor as `0x${string}`,
    baseSepolia.id,
  );

  const submissionTxns = results.items.filter(
    (tx: any) => tx.to?.hash === addressEditor,
  );

  console.dir(submissionTxns, { depth: null });

  for (const tx of submissionTxns) {
    const rawInput = tx.raw_input; // Assuming 'input' contains the raw input data

    console.log("Decoded Input:", fromHex(rawInput, "string"));
  }

  return submissionTxns.map((tx) => {
    return {
      from: tx.from.hash,
      hash: tx.hash,
      url: fromHex(tx.raw_input, "string"),
    };
  });
};
