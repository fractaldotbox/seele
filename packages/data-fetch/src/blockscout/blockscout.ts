...
const chainIdToApiRoot: any = {
  ...
  [chains.baseSepolia.id]: "https://base-sepolia.blockscout.com/api/",
...
export const getEndpointStrategy = (chainId?: number) => ({
  ...
  [BlockscoutEndpoint.Blob]: (params: BlockscoutEndpointParams) => {
    ...
    return (
      chainIdToApiRoot[chainId || chains.mainnet.id] + "v2/blobs/" + blobHash
    );
  },
});
...
export const getTransactionsWithAddress = async (
  address: Address,
  chainId?: number,
) => {
  const endpoint = getEndpoint(
...
export const getBlob = (blobHash: string, chainId: number) => {
  const endpoint = getEndpoint(BlockscoutEndpoint.Blob, { blobHash }, chainId);
